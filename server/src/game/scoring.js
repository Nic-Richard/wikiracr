const MAX_SCORE = 1_200_000;
const SCORE_TIME_CAP_SECONDS = 300;
const SCORE_CLICK_CAP_OVER_PAR = 10;
const SCORE_CLICK_WEIGHT = 0.6;
const SCORE_TIME_WEIGHT = 0.4;
const KNOCKOUT_MAX_DAMAGE = 1000;

function clamp01(value) {
  return Math.max(0, Math.min(1, value));
}

function scoreParts(clicks, optimalClicks, timeSeconds) {
...
function calcScore(clicks, optimalClicks, timeSeconds) {
  const parts = scoreParts(clicks, optimalClicks, timeSeconds);
  return Math.max(0, Math.min(MAX_SCORE, parts.clickScore + parts.timeScore));
}

function calcKnockoutDamage(scoreGap, cutoffScore, ramp = 1) {
  const safeScore = Math.max(0, cutoffScore || 0);
  if (safeScore <= 0) return 0;
  const damagePercent = clamp01(Math.max(0, scoreGap) / safeScore);
  return Math.round(damagePercent * KNOCKOUT_MAX_DAMAGE * ramp);
}

module.exports = {
  MAX_SCORE,
  SCORE_TIME_CAP_SECONDS,
  SCORE_CLICK_CAP_OVER_PAR,
  SCORE_CLICK_WEIGHT,
  SCORE_TIME_WEIGHT,
  KNOCKOUT_MAX_DAMAGE,
  calcScore,
